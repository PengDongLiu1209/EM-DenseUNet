...
def build_model(name: str, num_classes: int, default_model_config: dict):
    ...
    elif name == 'DenseUNet_HPA':
        return DenseUNet_HPA(n_classes=num_classes)
    elif name == 'DenseUNet_LCA':
        return DenseUNet_LCA(n_classes=num_classes)
    elif name == 'DenseUNet_SFA':
        return DenseUNet_SFA(n_classes=num_classes)
    elif name == 'DenseUNet_TSSA':
        return DenseUNet_TSSA(n_classes=num_classes)
    elif name == 'DenseUNet_FFCM':
        return DenseUNet_FFCM(n_classes=num_classes)
    elif name == 'DenseUNet_EFC':
        return DenseUNet_EFC(n_classes=num_classes)
    elif name == 'DenseUNet_SMFA':
        return DenseUNet_SMFA(n_classes=num_classes)
    elif name == 'DenseUNet_CDFA':
        return DenseUNet_CDFA(n_classes=num_classes)
    elif name == 'H_vmunet':
...
def main(args):
    ...
    train_path = f'/public/home/liupengdong2024/{dataset_name}/data_train'
    val_path = f'/public/home/liupengdong2024/{dataset_name}/data_val'
    test_path = f'/public/home/liupengdong2024/{dataset_name}/data_test'
    num_classes = 3
    ...
    if args.loss == 'CE':
        bce_pos_weight = torch.tensor([1.0, 1.0, 3.0], dtype=torch.float32).to(device)
        label_smoothing = 0.3 # 标签平滑
        criterion = torch.nn.CrossEntropyLoss(weight=bce_pos_weight,label_smoothing=label_smoothing)
    elif args.loss == 'BCDE':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', default='data_demo', type=str)
